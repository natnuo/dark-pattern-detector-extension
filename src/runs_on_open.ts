...
const fix = async (items: string[]): Promise<string[]> => {
  const result: any = await post("check", {
  ...
  for (let i=0;i<items.length;i++) {
    ...
    if (maxDP.type !== "None") console.log(maxDP, items[i], typeToColor[maxDP.type]);
...
document.querySelector("html")?.appendChild(loadingBar);
...
let lastUPD = -1;
const fixAll = async () => {
  if (Date.now() - lastUPD <= 301000) return;
  ...
  const items: [string, HTMLElement][] = (await Promise.all(cnlist.map(async (childNodes) => {
    let st="";
  ...
  }))).filter((vv) => { return vv!==undefined&&vv[0]!==""; }) as [string, HTMLElement][];  // casts are valid typescript bein werd mhmhmhmhm

  let itemssubgroups: [string, HTMLElement][][] = [];
  for (let i=0;i<items.length;i++) {
    if (i%10==0) itemssubgroups.push([items[i]]);
    else itemssubgroups[itemssubgroups.length - 1].push(items[i]);
  }
  ...
  for (let i=0;i<res.length;i++) {
    ...
    if (newColor === "transparent" || !cpe) continue;
